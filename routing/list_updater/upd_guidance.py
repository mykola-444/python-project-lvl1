...
class SpartaGuidanceStream:
    ...
    def get_failed_tests_details(reports):
        ...
        for report in reports:
            ...
            details = Data.get_test_details(content,
                                            Lib.GUIDANCE_FILE_NAME,
                                            Lib.GUIDANCE_DATA,
                                            Lib.GUIDANCE_NODE_ID)
    ...
    @staticmethod
    def get_list_of_failed_guidance_tests(change_id, jenkins_job, key, job_url):
        links = list()
        if job_url:
        ...
        if links:
            ...
            reports = Data.get_test_xml_reports(Lib.GUIDANCE_REPORT_PREFIX)
            log.info(" Tests reports: {}".format(reports))
            details = SpartaGuidanceStream.get_failed_tests_details(reports)
        else:
    ...
    def run_calculation(self):
        geo_data = self.get_list_of_failed_guidance_tests(self.change_id, self.jenkins_job, self.key, self.job_url)
